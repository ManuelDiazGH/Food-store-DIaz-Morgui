...
export function ProfileView() {
  ...
  if (isLoading) return <p className="text-gray-400">Cargando perfil...</p>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <p className="mt-1 text-lg text-gray-900">{perfil.nombre}</p>
        </div>
        <div>
        ...
        <div>
          ...
          <p className="mt-1 text-lg text-gray-900">{perfil.telefono || 'No especificado'}</p>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-500">Fecha de registro</label>
          <p className="mt-1 text-lg text-gray-900">{new Date(perfil.fecha_registro).toLocaleDateString('es-AR')}</p>
        </div>
      </div>
    </div>
  )
